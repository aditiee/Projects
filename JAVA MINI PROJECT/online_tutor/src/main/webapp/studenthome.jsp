...
<body>
	...
	<div class="container text-center my-3  bg-light"
		style="margin-top: 100px; height: 100%;">

		<div class="row mx-auto my-auto bg-light">
			<%
				for (StudentMapping tutors : tutorList) {
			%>
			<div class="col-md-3 bg-light"" id="1" >
				...
				<div class="card card-body" style="height:450px;">
					...
					<div class="card-body">
						...
						<p class="card-text">
							<strong style="font-size: 15px"><%=tutors.getTrainingName()%></strong>
						</p>
						<p class="card-text">
							<strong style="font-size: 10px"><i>by <%=tutors.getFirstName()%></i>
								<%=tutors.getLastName()%></strong>
						</p>
						<p style="text-align: left"><%=tutors.getDescription()%></p>

						<p class="card-text" style="text-align: right; font-size: 10px">
							<strong>created on : </strong>
							<%=tutors.getCreatedOn()%></p>
						<a
							href="studentrequest?trainingInfoId=<%=tutors.getTraining_info_id()%>&tutorId=<%=tutors.getTutorId() %>"
							class="btn" style="background: #5372F0; color: #fff;">Get
							Started</a>
